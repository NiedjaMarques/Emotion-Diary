...
const LoginForm = () => {
  ...
  const loginSubmit = async (e) => {
    e.preventDefault();
    // Aqui você pode enviar os dados do formulário para uma API ou outra lógica de manipulação
    ...
    try {
      ...
      if (result.error) {
        ...
        console.log("deu certo, bb", +result.email);
      }
    } catch (error) {
      ...
      <div className="h-screen flex items-center justify-center bg-slate-900">
        <div className="p-5 rounded-lg border-2 shadow-lg bg-white">
          ...
          <p className="text-sm text-center my-2">
            Não tem conta ?{" "}
            <Link className="hover:underline text-azulClaro" to="/cadastrar">
              Cadastre-se
            </Link>
          </p>
        </div>
